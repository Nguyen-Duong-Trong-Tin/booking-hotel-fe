        ...
        <div className="w-full max-w-7xl mx-auto flex items-center justify-between px-6">
          <div className="flex items-center gap-10">
            <Link to="/" className="hover:opacity-80 transition-all">
              <Title level={3} className="!mb-0 !text-cyan-700 font-black tracking-tighter">
                Booking Hotel
              </Title>
            </Link>
              ...
              {headerLinks.map((item) => {
                ...
                if (item.to) {
                  return (
                    <NavLink
                      ...
                      to={item.to}
                      className={({ isActive }) => 
                        `relative py-2 font-semibold transition-all duration-300 hover:text-cyan-700 ${
                    ...
                    >
                      {({ isActive }) => (
                        <>
                          ...
                          <span 
                            className={`absolute bottom-0 left-0 h-0.5 bg-cyan-600 transition-all duration-300 ${
                              isActive ? "w-full" : "w-0"
                            }`} 
                          />
                        </>
                ...
                return (
                  <a
                    key={item.label}
                    href={item.href}
                    className="text-slate-500 hover:text-cyan-700 font-semibold transition-all"
                  >
                    {item.label}
                  </a>
                );
          ...
          <Space>
            {isSignedIn ? (
              ...
              <Link to="/login">
                <Button
                  type="primary"
                  className="rounded-full px-6 font-bold bg-cyan-600 hover:bg-cyan-700 border-none shadow-md shadow-cyan-100"
                >
                  Sign in
                </Button>
              </Link>
